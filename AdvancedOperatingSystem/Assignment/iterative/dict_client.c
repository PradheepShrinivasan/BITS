...
int main (int argc, char *argv[])
{
	...
	while (1) {
		...
		switch (cmd) {
		case 'I':	/* "initialize" */
			result = initw_1((void*)&initw_1_arg, clnt);
			if (result == (int *) NULL) {
		...
		case 'i':	/* "insert" */
			...
			if (getword (word))
			{
				...
				if (getword (meaning))
				{
					...
					result = insertw_1(&currentWord, clnt);
					if (result == (int *) NULL) {
						clnt_perror (clnt, "call failed");
						break;
					}
		...
		case 'l':	/* "lookup" */
			...
			if (getword (word))
			{
				char **r_meaning = lookupw_1(&word, clnt);
				if (r_meaning == (char **) NULL) {
					clnt_perror (clnt, "call failed");
					break;
				}
				if (*r_meaning)
